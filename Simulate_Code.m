...
measurement_noise_std = 0.75; % Sensor noise standard deviation
...
while advance(scenario)
    ...
    true_position_2d = true_position_3d(1:2); % Actual Position (Without Error)
    
    noise_vec = randn(1, 2) * measurement_noise_std; % Random Noise Generation
    measured_position_2d = true_position_2d + noise_vec; % Position observed by the sensor (With Error) - GPS simulation
...
R = [measurement_noise_std^2  0; % motion model parameter (Nonlinear)
...
x_k = [measured_history(1, 1);
       ...
       30;
       0;
       0]; % Initial position and initial velocity
...
% Main EKF Loop - motion model (CRTV)
for k = 2:length(time_history) % Starting the loop for real-time data processing
    ...
    P_k_predict = Fj * P_prev * Fj' + Q; % Correction Step
...
xlabel('X Position (m)');
ylabel('Y Position (m)');
title('EKF Tracking Performance');
...
% Calculate Metrics - measurment model (Linear)
fprintf('\n Performance Metrics \n');

pos_true = true_history(:, 1:2); % Separating X and Y coordinates from storage matrices
pos_measured = measured_history(:, 1:2);
pos_filtered = kalman_history(:, 1:2);

error_noisy_vec = sqrt(sum((pos_measured - pos_true).^2, 2)); % Euclidean Distance
...
bias_noisy = mean(raw_error_noisy); % Bias or systematic deviation
bias_filtered = mean(raw_error_filtered);

fprintf('Metric:          Noisy Measurment Error.   Filtered Estimation Error.\n');
fprintf('RMSE:            %.4f m              %.4f m\n', rmse_noisy, rmse_filtered);
fprintf('MAE :            %.4f m              %.4f m\n', mae_noisy, mae_filtered);
fprintf('MAX :            %.4f m              %.4f m\n', maxe_noisy, maxe_filtered); % The worst moment of simulation

fprintf('\nFilter Bias (X, Y):\n');
fprintf('  Noisy:       [%.3f m, %.3f m]\n', bias_noisy(1), bias_noisy(2)); % Because noise is random, sometimes left, sometimes right, they eventually cancel each other out
fprintf('  Filtered:    [%.3f m, %.3f m]\n', bias_filtered(1), bias_filtered(2)); % The filter estimate falls slightly "behind" the actual path.

improvement_percent = 100 * (rmse_noisy - rmse_filtered) / rmse_noisy;
fprintf('\nOverall RMSE Improvement: %.2f %%\n', improvement_percent); % for example 50.41 %, when we use only kalman filter the error is 100% and when we use its with EKF, the error decrease 50.41 %